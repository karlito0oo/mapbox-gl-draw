...
DrawPolygon.onMouseMove = function (state, e) {
  ...
  const sidesMeasurements = [];

  const currentMouseLoc = [e.lngLat.lng, e.lngLat.lat]; // Corrected order
  ...
  if (coordinates.length > 1) {
    for (let i = 0; i < coordinates.length - 1; i++) {
      ...
      const distanceSide = turf.distance(startPoint, endPoint, {
        units: "feet",
      });
  ...
  if (map && map.getSource("measurement-point")) {
    map.getSource("measurement-point").setData({
      ...
      features: coordinates.slice(0, -1).map((coord, index) => {
        const midpoint = [
          (coord[0] +
